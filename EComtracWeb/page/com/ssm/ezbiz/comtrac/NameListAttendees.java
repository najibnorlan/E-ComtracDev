...
public class NameListAttendees extends SecBasePage {
	...
	public class SearchParticipantForm extends Form implements Serializable {
		public SearchParticipantForm(String id, IModel m, final RobTrainingConfig robTrainingConfig) {
			...
			add(new SSMLabel("internalUser", UserEnvironmentHelper.getUserenvironment().getFullName()));
			add(new SSMLabel("internalUserEmail", UserEnvironmentHelper.getUserenvironment().getEmail()));

			InternalUserEnviroment internalUser = (InternalUserEnviroment) UserEnvironmentHelper.getUserenvironment();
			System.out.println("Internal Email=" + internalUser.getEmail() + internalUser.getFullName());
		}
		...
		public void populateData(AjaxRequestTarget target, final RobTrainingConfig robTrainingConfig, String ic) {
			...
			SSMAjaxLink emailBlast = new SSMAjaxLink("emailBlast") {
				...
				public void onClick(AjaxRequestTarget target) {
					...
					String emailInternalUser = UserEnvironmentHelper.getUserenvironment().getEmail();
					String nameInternalUser = UserEnvironmentHelper.getUserenvironment().getFullName();
					
					try {
						mailService.sendMail(emailInternalUser,
								"email.notification.ecomtrac.internal.eligible.subject",
								robTrainingConfig.getTrainingCode(),
								"email.notification.ecomtrac.internal.eligible.body", nameInternalUser,
								robTrainingConfig.getTrainingName(),
								robTrainingConfig.getTrainingCode(),
								trainingDt);
						
						mailService.sendMail(emailInternalUser,
								"email.notification.ecomtrac.internal.noteligible.subject",
								robTrainingConfig.getTrainingCode(),
								"email.notification.ecomtrac.internal.noteligible.body", nameInternalUser,
								robTrainingConfig.getTrainingName(),
								robTrainingConfig.getTrainingCode(),
								trainingDt);
					} catch (Exception e) {
